...
export default function Fleet() {
  const { data: vehicleData, isSuccess: vehicledataFetched } =
    useGetVehiclesQuery();
  ...
  useEffect(() => {
    ...
    result = result.filter(
      (car) => car.price >= range[0] && car.price <= range[1]
    );
  ...
  const getModelsByBrand = (brand) => {
    return [
      ...new Set(
        vehicles.filter((car) => car.brand === brand).map((car) => car.model)
      ),
    ];
  };
  ...
  return (
    <div className="relative min-h-screen px-4 pt-24">
      ...
      <div className="flex flex-col lg:flex-row gap-x-5">
        {/* Filtering system */}
        <div
          className={`fixed rounded-2xl h-full lg:relative top-0 left-0 w-full lg:w-1/4 bg-black text-white p-4 overflow-y-auto scrollbar-hide transition-transform duration-300 ease-in-out z-40 ${
            isFilterVisible
              ? "translate-x-0"
        ...
        >
          ...
          <div className="flex justify-between gap-3 mb-5">
            ...
            >
              ...
              {filters.brand &&
                getModelsByBrand(filters.brand).map((model) => (
