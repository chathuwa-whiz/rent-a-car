  ...
  return (
    <div className="relative w-full h-screen bg-gradient-to-b from-black via-[#1c1b1b] to-white md:bg-gradient-to-r md:from-black md:via-[#626262] md:to-white flex flex-col md:flex-row items-center justify-between px-6 md:px-16 overflow-hidden">
      ...
      <div className="flex flex-row items-center justify-center md:justify-start gap-x-10 md:gap-x-16 w-full mt-20 md:mt-0 md:ml-4">
        ...
        {/* Social Icons */}
        <div className="flex flex-col mt-4 md:mt-10 space-y-11 md:space-y-23 text-graydark">
          ...
          <FaWhatsapp className="text-lg md:text-xl cursor-pointer hover:text-graylight transition" />
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <h1 className="text-4xl sm:text-6xl md:text-7xl lg:text-8xl mt-4 font-bold uppercase leading-none tracking-widest">
              Luxury <br /> Lifestyle <br /> Rentals
            </h1>

            {/* Decorative Lines */}
            <div className="flex justify-start gap-2 mt-4 mb-6">
              <div className="w-16 sm:w-20 h-[6px] sm:h-[8px] bg-white rounded-full"></div>
              <div className="w-4 sm:w-6 h-[6px] sm:h-[8px] bg-white rounded-full"></div>
              <div className="w-4 sm:w-6 h-[6px] sm:h-[8px] bg-white rounded-full"></div>
              <div className="w-12 sm:w-14 h-[6px] sm:h-[8px] bg-white rounded-full"></div>
            </div>

            {/* Pricing Section (Auto-Switching Car Details) */}
            <div className="mt-2 transition-opacity duration-500 ease-in-out">
              <p className="text-xl sm:text-2xl font-bold pb-2">{carData[currentCar].name}</p>
              <p className="text-darkred text-lg sm:text-xl font-bold inline tracking-wider">
                {carData[currentCar].price}
              </p>
              <span className="text-sm sm:text-lg font-bold text-graylight md:text-graydark"> / Per Day</span>
            </div>

            {/* Booking Button */}
            <button className="mt-6 sm:mt-12 px-8 sm:px-12 py-2 sm:py-3 border-2 cursor-pointer border-gasolindark text-sm sm:text-lg font-semibold rounded-[2px] hover:bg-gasolinlight hover:border-gasolinlight transition">
              Book Now
            </button>
          </motion.div>
        </AnimatePresence>
      ...
      <div className="absolute bottom-0 -md:bottom-10 md:-right-20 w-full md:w-[55rem] overflow-hidden">
        <AnimatePresence mode="wait">
          <motion.img
            key={currentCar}
