...
export default function Rentals() {
  ...
  return (
    <div className="text-white px-3 sm:px-6 pb-6">
      {/* Active Rentals */}
      <div className="bg-black px-3 sm:px-6 py-3 rounded-lg w-full">
        ...
        )) : (<div className="text-white text-center">No active rentals</div>)}
      </div>

      {/* Rental History */}
      <div className="mt-6 bg-black px-3 sm:px-6 py-3 rounded-lg w-full">
        <h3 className="text-darkred font-semibold text-xl sm:text-2xl">Rental History</h3>
        <div className="mt-4 space-y-4 px-2 sm:px-10">
          {bookings.length > 0 ? sortedBookings.map((rental, index) => (
            <div
              key={index}
              className="flex flex-col sm:flex-row items-center justify-between py-4 px-3 sm:px-10 rounded-lg border border-gasolindark gap-4 sm:gap-0"
            >
              <img
                src={rental.vehicle.primaryImage}
                alt={rental.vehicle.model}
              ...
              <div className="text-center sm:text-left">
                <h4 className="text-xl sm:text-2xl font-bold">{rental.vehicle.brand}</h4>
          ...
          )) : <h3 className="text-darkred text-center text-lg sm:text-xl">No rental history</h3>}
        </div>
      </div>
    </div>
  );
